...
VEER = Token:match("(%d+)")
redis:set(VEER..":VERSION",GetUser.information.Source_version)
redis:set(VEER..":SUDO_ID:",GetUser.information.id)
redis:set(VEER..":DataCenter:",GetUser.information.DataCenter)
...
VEER = Token:match("(%d+)")
our_id = tonumber(VEER)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(VEER..":UserNameBot:")
SUDO_ID = tonumber(redis:get(VEER..":SUDO_ID:"))
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..VEER..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(VEER..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(VEER..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(VEER..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(VEER..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور 👨🏽‍💻'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(VEER..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء 👷🏽'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(VEER..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير 👨🏼‍⚕️' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(VEER..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن 👨🏼‍🎓'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(VEER..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
if msg.Admin then redis:srem(VEER..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(VEER..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(VEER..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.VEER then 
if msg.text and ScriptFile.iVEER then
for k, VEER in pairs(ScriptFile.VEER) do
local SearchText = msg.text:match(VEER)
if SearchText then
if not CheckFlood(msg) then
return false 
end
local GetMsg = ScriptFile.iVEER(msg,{msg.text:match(VEER)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",VEER.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dVEER then
if ScriptFile.dVEER(msg) == false then
return false
end
...
if Plug.VEER then 
if msg.text and Plug.iVEER then
for k, VEER in pairs(Plug.VEER) do
local SearchText = msg.text:match(VEER)
if SearchText then
if not CheckFlood(msg) then
return false
end
local GetMsg = Plug.iVEER(msg,{msg.text:match(VEER)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",VEER.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source VEER")
end 

end
else
print("The File Script.lua Not Runing in The Source VEER")
...
function tdcli_update_callback(data)
	...
	redis:srem(VEER..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/VEERLITE/VEER/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/VEERLITE/VEER/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/VEERLITE/VEER/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/VEERLITE/VEER/master/inc/functions.lua','./inc/functions.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'100% - |█████████████|\n\n🔝*¦* تم تحديث السورس الى اصدار *v'..redis:get(VEER..":VERSION")..'*\n📟*¦* تم اعاده تشغيل السورس بنجاح')
	dofile("./inc/Run.lua")
	...
	if redis:get(VEER..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(VEER..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(VEER..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(VEER..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(VEER..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(VEER..'lock_bots'..msg.chat_id_)
	ISBOT = false
	ZISBOT = false
	...
	if ZISBOT and redis:get(VEER..'lock_bots_by_kick'..msg.chat_id_) then
	...
	if redis:get(VEER..'group:add'..data.chat_.id_) then
	redis:set(VEER..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(VEER..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(VEER..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(VEER..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	VEER..'group:name'..k,IDS.Title,
	VEER..'num_msg_max'..k,5,
	VEER..'group:add'..k,true,
	VEER..'lock_link'..k,true,
	VEER..'lock_spam'..k,true,
	VEER..'lock_webpage'..k,true,
	VEER..'lock_markdown'..k,true,
	VEER..'lock_flood'..k,true,
	VEER..'lock_bots'..k,true,
	VEER..'mute_forward'..k,true,
	VEER..'mute_contact'..k,true,
	VEER..'mute_document'..k,true,
	VEER..'mute_inline'..k,true,
	VEER..'lock_username'..k,true,
	VEER..'replay'..k,true
	)
	redis:sadd(VEER..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(VEER..'username:'..ID,'username',user)
	redis:sadd(VEER..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(VEER..'username:'..ID,'username',user)
	redis:sadd(VEER..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(VEER..'username:'..ID,'username',user)
	redis:sadd(VEER..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(VEER..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(VEER..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(VEER..'username:'..data.user_.id_).username
	...
	redis:hset(VEER..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
